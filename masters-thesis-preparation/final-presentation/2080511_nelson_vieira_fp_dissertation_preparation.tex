...
\begin{frame}{Differential Privacy}
    \begin{figure}
        \centering\includegraphics[width=200pt]{assets/images/dp.png}
...
\begin{frame}{Literature Approaches}
    \begin{multicols}{2}
        ...
        \columnbreak
        \begin{itemize}
            \only<1>{\item Interactive theatre experience \cite{SkirpanPrivacy}}
            \only<2>{\item Communication strategy for a remote-control system \cite{SunSecure}}
            \only<3>{\item Domain-specific ontology for modeling IoT security and privacy policies \cite{opara2022framework}}
            \only<4>{\item Software stack that combines peer-to-peer file sharing with blockchain smart contracts \cite{AliIoT}}
        \end{itemize}
    \end{multicols}
\end{frame}
