    ...
    \begin{center}
        \begin{tikzpicture}
            \begin{axis}[
                ...
                nodes near coords={\pgfmathprintnumber\pgfplotspointmeta\%},
                every node near coord/.append style={rotate=90, anchor=west},
                legend style={at={(0.85,-0.35)},anchor=north}
            ]
...
On Figure \ref{fig:system_usability_scale_scores} the scores of
each participant can be seen.
The 90,95 score was calculated by using the following formula:
\[
    Score = \frac{\sum_{i=1}^{7}x_i}{7}
\]
where $x$, representing each participants' response, is noted as:
\[
